                ...
                <th
                    ...
                    <slot
                        ...
                        <input
                                ...
                                type="search">
                        <div v-else-if="!columnSearch" v-html="field.label"/>
                    </slot>
                </th>
            </tr>
            </tfoot>
...
export default {
  ...
  created () {
    ...
    that.options.order = that.options.order || [[startCol, 'desc']]
    if (that.fields) {
      const fields = that.fields
      const cols = that.options.columns
      const orders = that.options.order
    ...
    if (startCol > 0) {
      if (that.options.order) {
        ...
        that.options.order = [[startCol, 'desc']]
      }
    }
  },
  mounted () {
    ...
    if (that.details) {
      ...
      if (that.details.template || that.$scopedSlots.details) {

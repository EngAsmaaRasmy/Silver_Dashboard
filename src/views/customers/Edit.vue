      ...
      <CRow>
        <CCol md="12">
          <CCard>
           ...
           <CCardHeader>
            <CCol col="6" style="font-size: 25px;"> {{ $t('message.update_customer') }}</CCol>
          </CCardHeader>
          <CCardBody>
            <CRow>
              <CCol sm="6">
  ...
  methods: {
    ...
    formData.append('name', this.customer.name)
      ...
      formData.append('_method', 'PUT')
       ...
       this.errors = await this.postRequest(formData, `${this.$hostUrl}admin-dashboard/customers/${this.$route.params.id}`, '/customers')
    },
    goBack () {

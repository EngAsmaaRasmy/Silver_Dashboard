...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          url:  this.$hostUrl + 'admin-dashboard/customers',
          dataSrc: (json) => {
            console.log(json)
            return json.data
          }
  ...
  methods: {
    doLoadTable (cb) {
      this.$http.getJSON(
        process.env.VUE_APP_BACKEND_SERVER_2 + 'admin-dashboard/customers',
        function (data) {
          cb(data.data)
        }
      )
    ...
    doAlertShow (data) {
      this.$router.push({ path: `customers/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/customers/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}admin-dashboard/customers/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {

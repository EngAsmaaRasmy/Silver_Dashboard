  ...
  <div>
    <CRow>
      <CCol col="12">
          ...
          <CCardHeader>
            <CCol col="6" style="font-size: 25px;"> {{ $t('message.update_employee') }}</CCol>
          </CCardHeader>
          <CCardBody>
             <CRow>
              <CCol sm='6'>
...
export default {
  ...
  methods: {
    createEmployee:function (e) {
      ...
      this.postRequest(formData, `${this.$hostUrl}employees/${this.$route.params.id}`, '/employees')
    },
    goBack () {
      this.productsOpened ? this.$router.go(-1) : this.$router.push({ path: '/employees' })
    }
  }
}

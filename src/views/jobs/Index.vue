        ...
        <CCardHeader>
          <CRow>
            ...
            <CCol col="6" class="cardButton">
                <CButton  to="/jobs/create"  square color="primary">
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp;  {{$t('message.create_job')}}
                </CButton>
            </CCol>
        </CRow>
        </CCardHeader>
...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          //   )
          // },
          url: 'http://178.79.181.70:8003/api/jobs',
          dataSrc: (json) => {
            return json.data
          }
        },
  ...
  methods: {
    doLoadTable (cb) {
      this.$http.getJSON(
        'http://178.79.181.70:8003/api/jobs',
        function (data) {
          cb(data.data)
        }
      )
    ...
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `http://178.79.181.70:8003/api/jobs/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {

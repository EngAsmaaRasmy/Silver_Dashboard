          ...
          <CRow>
            <CCol style="font-size: 25px;" col="6">
             ...
             <CCol col="6" style="text-align: left;" class="cardButton">
                <CButton  to="/order-statuses/create"  square color="primary">
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp;  إضافة حالة طلب جديدة
                </CButton>
            </CCol>
        </CRow>
        </CCardHeader>
...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          url: this.$hostUrl + 'order-statuses',
          dataSrc: (json) => {
            return json.data
          }
        },
  ...
  methods: {
    ...
    doAlertShow (data) {
      this.$router.push({ path: `order-statuses/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/order-statuses/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}order-statuses/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {
              console.log(response)
              localStorage.ordertatuses = Number(localStorage.orderStatuses) - 1
              jquery('#order-statuses_badge').text(localStorage.orderStatuses)

            ...
            <CCol style="font-size: 25px;" col="6">
             ...
             <CCol col="6"  class="cardButton">
                <CButton  to="/products/create"  square color="primary">
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp; {{$t('message.create_product')}}
                </CButton>
            </CCol>
        </CRow>
        </CCardHeader>
...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          url: 'http://178.79.181.70:8003/api/products',
          dataSrc: (json) => {
            return json.data
          }
        },
      ...
      fields: {
        name: {
          label: this.$t('message.productName'),
          sortable: true,
          searchable: true,
  ...
  methods: {
    doLoadTable (cb) {
      this.$http.getJSON(
        'http://178.79.181.70:8003/api/products',
        function (data) {
          cb(data.data)
        }
      )
    ...
    doAlertShow (data) {
      this.$router.push({ path: `products/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
    ...
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `http://178.79.181.70:8003/api/products/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {

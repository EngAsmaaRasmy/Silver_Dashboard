        ...
        <CCardHeader>
          <CRow>
            <CCol style="font-size: 25px;" col="6">
                {{$t('message.customers_accounts')}}
            </CCol>
             <!-- <CCol col="6" class="cardButton">
                <CButton  to="/customers/Create"  square color="primary">
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp;   {{$t('message.create_customer')}}
                </CButton>
            </CCol> -->
        </CRow>
        </CCardHeader>
...
export default {
    ...
    return {
      options: {
        ajax: {
          beforeSend: function (xhr) {
            ...
            xhr.setRequestHeader(
              'x-api-key',
              process.env.VUE_APP_MS_TOKEN
            )
          },
          headers: new Headers({
            'x-api-key': process.env.VUE_APP_MS_TOKEN
          }),
          url: process.env.VUE_APP_BACKEND_SERVER + 'admin-dashboard/customers-acounts',
          dataSrc: (json) => {
            console.log(json)
            return json.data
          }
      ...
      fields: {
        customer: {
         ...
         typecustomer: {
          label: this.$t('message.type_of_customer'),
          sortable: true,
          searchable: true,
          defaultOrder: 'SORT_ASC '
        },
  ...
  methods: {
    doLoadTable (cb) {
      this.$http.getJSON(
        process.env.VUE_APP_BACKEND_SERVER_2 + 'admin-dashboard/customers-acounts',
        function (data) {
          cb(data.data)
        }
      )
    ...
    doAlertShow (data) {
      this.$router.push({ path: `customers-accounts/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/customers-accounts/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}admin-dashboard/customers-acounts/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {
              console.log(response)
              localStorage.users = Number(localStorage.users) - 1
              jquery('#users_badge').text(localStorage.users)

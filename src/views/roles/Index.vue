          ...
          <CRow>
            <CCol style="font-size: 25px;" col="6">
             ...
             <CCol col="6" style="text-align: left;" class="cardButton">
                <CButton  to="/roles/Create"  square color="primary">
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp;   {{$t('message.create_role')}}
                </CButton>
            </CCol>
        </CRow>
        </CCardHeader>
...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          url: process.env.VUE_APP_BACKEND_SERVER_2 + 'roles',
          dataSrc: (json) => {
            return json.data
          }
        },
  ...
  methods: {
    doLoadTable (cb) {
      alert('i do')
      this.$http.getJSON(
        process.env.VUE_APP_BACKEND_SERVER_2 + 'roles',
        function (data) {
          cb(data.data)
          console.log(`deleting item ID: ${data.name}`)
        }
      )
    },
    doAlertShow (data) {
      this.$router.push({ path: `roles/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/roles/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${process.env.VUE_APP_BACKEND_SERVER_2}roles/${this.$route.params.id}`,
              { _method: 'delete' }
            )
            .then((response) => {
              console.log(response)
              localStorage.roles = Number(localStorage.roles) - 1
              jquery('#roles_badge').text(localStorage.roles)

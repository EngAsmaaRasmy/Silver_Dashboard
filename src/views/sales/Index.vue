        ...
        <CCardHeader>
          <CRow>
            <CCol style="font-size: 25px;" col="6">
                {{$t('message.sales_invoices')}}
            </CCol>
             <CCol col="6" style="text-align: left;" class="cardButton">
                <CButton  to="sales-invoices/create"  square color="primary">
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp;  {{$t('message.create_sales_invoices')}}
                </CButton>
            </CCol>
        </CRow>
        </CCardHeader>
...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          url: this.$hostUrl + 'admin-dashboard/sale-invoices',
          dataSrc: (json) => {
            return json.data
          }
        },
  ...
  methods: {
    doLoadTable (cb) {
      this.$http.getJSON(
        this.$hostUrl + 'admin-dashboard/sale-invoices',
        function (data) {
          cb(data.data)
        }
      )
    ...
    doAlertShow (data) {
      this.$router.push({ path: `/sales-invoices/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/sales-invoices/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    ...
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}admin-dashboard/sale-invoices/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {

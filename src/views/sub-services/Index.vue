        ...
        <CCardHeader>
          <CRow>
            <CCol style="font-size: 25px;" col="6">
                {{$t('message.subService')}}
            </CCol>
             <CCol col="6" style="text-align: left;" class="cardButton">
                <CButton  to="sub-services/create"  square color="primary">
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp;  {{$t('message.create_subService')}}
                </CButton>
            </CCol>
        </CRow>
        </CCardHeader>
...
export default {
  ...
  methods: {
    ...
    doAlertShow (data) {
      this.$router.push({ path: `sub-services/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/sub-services/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    ...
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}sub-services/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {

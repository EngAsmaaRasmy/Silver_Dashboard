        ...
        <CCardHeader>
          <CRow>
            ...
            <CCol col="6" class="cardButton" >
                <CButton  to="/suppliers/create"  square color="primary">
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp;  {{ $t('message.create_supplier')}}
                </CButton>
            </CCol>
        </CRow>
        </CCardHeader>
...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          //   )
          // },
          // url: this.$hostUrl + 'suppliers',
          url: 'http://178.79.181.70:8003/api/suppliers',
          dataSrc: (json) => {
            return json.data
          }
        },
      ...
      fields: {
        ...
        name: {
          label: this.$t('message.supplier_name'),
          sortable: true,
          searchable: true,
          defaultOrder: 'desc'
        },
        email: {
          label: this.$t('message.supplier_email'),
          sortable: true,
          searchable: true,
  ...
  methods: {
    ...
    doAlertShow (data) {
      this.$router.push({ path: `/suppliers/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/suppliers/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}suppliers/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {
